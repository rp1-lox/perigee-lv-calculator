  ...
  sc.legs.forEach((leg, li) => {
    ...
    if (isSun) {
      ...
      if (legFlags[leg.key] === 'second' && destBody && PROG_HELIO_R[destBody] != null) {
        ...
        const depP = { x: ox + depWorld.x * zoom, y: oy + depWorld.y * zoom }; // Sun frame: ox,oy IS the Sun's render pos; world km -> units via zoom
        ...
        const arrP = { x: ox + arrWorld.x * zoom, y: oy + arrWorld.y * zoom };
        ...
        const ghostMarkerSvg = (px, py) => {
          ...
          return `<circle cx="0" cy="0" r="${r.toFixed(2)}" fill="none" stroke="var(--text-dim)" stroke-width="0.8" stroke-dasharray="1.5,1.5" pointer-events="auto"><title>${ghostTitle}</title></circle>`;
        };
        _trajRegisterLabel(depP.x, depP.y, [{ text: `${leg.fromLabel} → ${leg.toLabel}`, dy: -4 - _trajBodyPxR(2.5, zoom), fontPx: 9, color: 'var(--text-dim)' }], 'zone',
          { screenSize: _trajBodyPxR(2.5, zoom), minSize: 0, selected: false, marker: ghostMarkerSvg(depP.x, depP.y), opacity: 0.7 * stateAlpha });
        ...
        _trajRegisterLabel(arrP.x, arrP.y, [{ text: `${destBody} (2nd leg)`, dy: -4 - _trajBodyPxR(2.5, zoom), fontPx: 9, color: 'var(--text-dim)' }], 'zone',
          { screenSize: _trajBodyPxR(2.5, zoom), minSize: 0, selected: false, marker: ghostMarkerSvg(arrP.x, arrP.y), opacity: 0.7 * stateAlpha });
        return;
      }
    ...
    if (leg.fromO && leg.fromO.body === 'Sun' && legFlags[leg.key] === 'second') {
      ...
      const ghostMarkerSvg = `<circle cx="0" cy="0" r="${gr.toFixed(2)}" fill="none" stroke="var(--text-dim)" stroke-width="0.8" stroke-dasharray="1.5,1.5" pointer-events="auto"><title>${ghostTitle}</title></circle>`;
      _trajRegisterLabel(depLocal.x, depLocal.y, [{ text: leg.fromLabel, dy: -4 - gr, fontPx: 9, color: 'var(--text-dim)' }], 'zone', { screenSize: gr, minSize: 0, selected: false, marker: ghostMarkerSvg, opacity: 0.7 * stateAlpha });
      ...
      _trajRegisterLabel(arrLocal.x, arrLocal.y, [{ text: `${leg.toLabel} (2nd leg)`, dy: -4 - gr, fontPx: 9, color: 'var(--text-dim)' }], 'zone', { screenSize: gr, minSize: 0, selected: false, marker: ghostMarkerSvg, opacity: 0.7 * stateAlpha });
      return;
    }
