...
// untilted behavior, so this is a mechanical, zero-behavior-change
// generalization for every body not in the pole table.
function _trajSpinRotate(pt, spinRad, body) {
  ...
  if (body) {
    ...
    return [
      ...
      ex * B.xEq[1] + ey * B.yEq[1] + ez * B.zEq[1],
      ex * B.xEq[2] + ey * B.yEq[2] + ez * B.zEq[2],
    ];
  }
  return [ex, ey, ez];
}
...
function _trajFrameBasisAt(frameKind, body, t) {
  ...
  if (frameKind === 'body-fixed') {
    ...
    // that product (xh/yh/zh are read as columns by _trajFrameTransform,
    // which computes q = pRel . xh / pRel . yh / pRel . zh, i.e. q = M^T
    // pRel for M's rows == xh/yh/zh as WRITTEN — verified against the
    // pre-§20-O2 identity-tilt case: xh=[c,s,0] etc. is exactly Tilt=I's
    // Rz(a) columns) are Tilt applied to Rz(a)'s own columns [c,s,0],
    // [-s,c,0], [0,0,1]: xh = c*xEq + s*yEq, yh = -s*xEq + c*yEq, zh = zEq.
    ...
    return {
      ...
      yh: [-s * xEq[0] + c * yEq[0], -s * xEq[1] + c * yEq[1], -s * xEq[2] + c * yEq[2]],
      zh: zEq,
    };
  }
...
function _trajGeoPolyPath(lonLatTenths, spinAngle, cx, cy, rPx, body) {
  const dirs = [];
  for (let i = 0; i < lonLatTenths.length; i += 2) {
    dirs.push(_trajSpinRotate(_trajLatLonUnit(lonLatTenths[i + 1] / 10, lonLatTenths[i] / 10), spinAngle, body));
  }
...
function _trajGeoEllipsePath(feat, spinAngle, cx, cy, rPx, body) {
  const n = 20;
  ...
  for (let k = 0; k < n; k++) { // n (not n+1): the polygon is implicitly closed by the clip/path Z, no duplicate seam point
    ...
    dirs.push(_trajSpinRotate(_trajLatLonUnit(lat, lon), spinAngle, body));
  }
  return _trajClippedRunsToPath(_trajHemiClipRuns(dirs), cx, cy, rPx);
}
  ...
  if (!skipGeometry) {
    if (body === 'Earth' && typeof PROG_GEO_EARTH !== 'undefined') {
      PROG_GEO_EARTH.forEach(poly => {
        const d = _trajGeoPolyPath(poly, spinAngle, cx, cy, rPx, body);
      ...
      (PROG_GEO_FEATURES.Moon || []).forEach(f => {
        const d = _trajGeoEllipsePath(f, spinAngle, cx, cy, rPx, body);
      ...
      for (let b = 0; b < nBands; b++) {
        ...
        for (let k = 0; k <= nSeg; k++) {
          ...
          dirs.push(_trajSpinRotate(_trajLatLonUnit(lat1, lon), spinAngle, body));
        }
        for (let k = nSeg; k >= 0; k--) {
          const lon = -180 + (360 * k) / nSeg;
          dirs.push(_trajSpinRotate(_trajLatLonUnit(lat0, lon), spinAngle, body));
        }
...
function _trajRasterGlobe(body, discPx, spinAngle, az, el, sunDir3, maxPx) {
  ...
  // then must rotate THAT into the body's EQUATORIAL frame (ex,ey,ez) before
  // un-spinning to recover lat/lon — the mirror-image step of the forward
  // spin-then-tilt composition. physEqBasis is orthonormal, so its inverse
  // is its transpose: eq = worldVec . (xEq | yEq | zEq) column-wise, i.e.
  ...
  for (let py = 0; py < size; py++) {
    ...
    for (let px = 0; px < size; px++) {
      ...
      const ey = x * eqY[0] + y * eqY[1] + z * eqY[2];
      const ez = x * eqZ[0] + y * eqZ[1] + z * eqZ[2];
      const bx = ex * spc + ey * sps;
      const by = -ex * sps + ey * spc;
      const bz = ez;
      ...
      if (cloudActive) {
        ...
        // pre-spin equatorial-frame direction (ex,ey,ez) with cloudSpin
        // instead of the body's own spinAngle. Latitude is spin-invariant.
        const ccx = ex * cspc + ey * csps;
        const ccy = -ex * csps + ey * cspc;
