...
> **STATUS (2026-07-17): C2 ADAPTERS DONE + writer renames LARGELY DONE** (MISSION_MODEL_V2.md §24 C2 / C2b as-built). `orbitNormalize` + `orbitMeanRadiusKm`/`orbitPeriodS`/`orbitWorldNormal` live in `src/js/384-orbit-canonical.js`; canonical shape `{ body, periKm, apoKm, incDeg, lanDeg, argpDeg?, frame }`. The C1 boundary (`orbitWorldElements`/`orbitWorldState`, 385) now normalizes internally. **Writers migrated to canonical**: `refOrbitResolve`/catalog store + add/update inputs (425, legacy-input tolerance now RETIRED), event `e.orbit` (570), `m.launchOrbit` seed default + its string-keyed setter (item 1), the 5746 orbit-inspector's `e.orbit`/refOrbit/`m.launchOrbit` writes (item 2). **Still node-map dialect** (`perigee/apogee/inclination/lan_deg`): 430 node-map catalog + `progNmComputeEdgeDv`, 570-mission-nodemap node specs, and 565's plane-solver node.orbit reads — these are INTERLOCKED (565 reads node.orbit's `.type`/`.destination`/`.c3` too, so it can't route through `orbitNormalize`) and are the remaining C2b item-3/4 work. **Correction to the table below**: the node-map/catalog + `refOrbitResolve` keplerian dialects use `lan`, NOT `lan_deg` (only launch-planner `plan.*` objects carry `lan_deg`) — `orbitNormalize` accepts both. **060-orbit-categories.js `ORBIT_CATEGORIES` is NOT part of this dialect** — it is the LV-calculator destinations catalog (`apogee/perigee/inc/parking/c3/decl`) on the frozen `calculate()`/`destOnOrbitDV` path, out of scope for the node-map rename.
