...
> **STATUS (2026-07-17): C2 FULLY DONE — adapters + ALL writer renames** (MISSION_MODEL_V2.md §24 C2 / C2b as-built). `orbitNormalize` + `orbitMeanRadiusKm`/`orbitPeriodS`/`orbitWorldNormal` live in `src/js/384-orbit-canonical.js`; canonical shape `{ body, periKm, apoKm, incDeg, lanDeg, argpDeg?, frame }`. The C1 boundary (`orbitWorldElements`/`orbitWorldState`, 385) normalizes internally. **All writers now canonical**: `refOrbitResolve`/catalog (425), event `e.orbit` (570), `m.launchOrbit` + its setter, the 5746 orbit-inspector writes, AND (final C2b sweep) the node-map dialect — `PROG_NM_NODES` builtin literals (430), `_missionOrbitToNodeOrbit`/`missionSaveCustomNode`/custom-node specs (570-mission-nodemap + 5746), and `e.orbitAtBurn` (570-mission-replay). node.orbit READERS (430/565×3/570/572/574/5741/5744/5745/5746) were made canonical-primary dialect-tolerant rather than pure-canonical, so the 288-assertion node-map ΔV goldens + plane-match/saros pins stay byte-stable with no physics-fixture churn (565's `fromO`/`toO` read node-only `.type`/`.destination`/`.c3` too, so they can't route through `orbitNormalize`; the interlock is why 430+565 flipped in one atomic pass). Persistence: `_missionMigrateOrbitFieldNames` (450) absorbs the node-map dialect + `_missionMigrateNodeMapCustomNodes` covers custom nodes → legacy blobs migrate on load; no legacy writer remains in scope. **Correction to the table below**: the node-map/catalog + `refOrbitResolve` keplerian dialects used `lan`, NOT `lan_deg` (only launch-planner `plan.*` objects carry `lan_deg`) — `orbitNormalize` accepts both. **060-orbit-categories.js `ORBIT_CATEGORIES` + 360 `PROG_BUILTIN_NODES` are a DIFFERENT dialect** (`apogee/perigee/inc/...`) on the frozen `calculate()`/`destOnOrbitDV` path — correctly out of scope, untouched.
